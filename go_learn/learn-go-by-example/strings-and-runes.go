...
func main() {
	...
	// values at each index. This loop generates the
	// hex values of all the bytes that constitute the
	// code points in s.
	for i := 0; i < len(s); i++ {
		fmt.Printf("%x", s[i])
	}
	...
	// sequentially. Some Thai characters are represented
	// by UTF-8 code points that can span multiple bytes,
	...
	// We can achieve the same iteration by using
