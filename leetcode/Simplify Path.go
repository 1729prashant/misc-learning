...
func (s *Solution) simplifyPath(path string) string {
    ...
    for _, directory := range strings.Split(path, "/") {
        switch directory {
        case "", ".":
            // Ignore empty strings and current directory indicators
        case "..":
    ...
    // Join the stack with '/' and return
...
func testCases(s *Solution) {
    ...
    for _, tt := range tests {
        if got := s.simplifyPath(tt.input); got != tt.want {
            fmt.Printf("simplifyPath(%q) = %q, want %q\n", tt.input, got, tt.want)
        }
    }
